...
| Entry-point plugin discovery | **Optional, behind the contributor protocol.** Not the hazard it was once called: what breaks determinism is nondeterministic *contribution*, not discovery. But "identical across workers" holds only for a homogeneous local run — xdist supports SSH and socket workers — so a heterogeneous session needs a registry manifest comparison or an explicit homogeneous-plugin requirement |
